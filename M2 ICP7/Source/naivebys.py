...
data = data.withColumnRenamed("age", "label").select("label", col("education-num").alias("education-num"), col(" hours-per-week").alias("hours-per-week"),col(" education").alias("education"),col(" fnlwgt").alias("fnlwgt"),col(" sex").alias("sex"))
data = data.select(data.label.cast("double"),"education-num", "hours-per-week","education","sex","fnlwgt")

new_data=data.toDF("label","education-num","hours-per-week","education","sex","fnlwgt")
...
indexed1=indexed1.drop("sex","education")
indexed1.show()


# Create vector assembler for feature columns
assembler = VectorAssembler(inputCols=indexed1.columns[1:], outputCol="features")
data = assembler.transform(indexed1)
